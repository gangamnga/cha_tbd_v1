...
export function TestimonyForm() {
  ...
  const handleReset = () => {
    ...
    setTitle(""); setContent(""); setError(null); setSubmitted(false);
  };
  ...
  if (submitted) {
    return (
      <div className="bg-white flex flex-col items-center justify-center gap-4 py-12 px-6 text-center rounded-lg min-h-[260px]">
        ...
        <h3 className="font-bold text-[18px] text-vatican-dark">Cảm ơn bạn đã gửi lời chứng!</h3>
        <p className="text-[18px] text-gray-600 leading-relaxed max-w-md">
          Lời chứng của bạn sẽ được Ban biên tập xét duyệt trước khi đăng.
        </p>
        <button type="button" onClick={handleReset}
          className="mt-2 text-[16px] font-bold text-vatican-blue hover:underline transition-colors">
          Gửi thêm lời chứng khác
        </button>
      </div>
    );
  }
    ...
    <form onSubmit={handleSubmit}
      ...
      <div className="flex flex-col sm:flex-row divide-y sm:divide-y-0 sm:divide-x divide-gray-100">
        <div className="flex-1 flex flex-col px-4 py-2.5 gap-1 focus-within:bg-blue-50/30 transition-colors">
          <label htmlFor="ol-name" className="text-[16px] font-bold uppercase tracking-wide text-gray-500">Họ và tên</label>
          <input id="ol-name" type="text" value={name} onChange={e => setName(e.target.value)}
            placeholder="Tên của bạn (hoặc bút danh)"
            className="text-[18px] text-vatican-dark placeholder:text-gray-300 outline-none bg-transparent" />
        </div>
        <div className="flex-1 flex flex-col px-4 py-2.5 gap-1 focus-within:bg-blue-50/30 transition-colors">
          <label htmlFor="ol-phone" className="text-[16px] font-bold uppercase tracking-wide text-gray-500">Số điện thoại</label>
          <input id="ol-phone" type="tel" value={phone} onChange={e => setPhone(e.target.value)}
            placeholder="0901 234 567"
            className="text-[18px] text-vatican-dark placeholder:text-gray-300 outline-none bg-transparent" />
        </div>
        <div className="flex-1 flex flex-col px-4 py-2.5 gap-1 focus-within:bg-blue-50/30 transition-colors">
          <label htmlFor="ol-location" className="text-[16px] font-bold uppercase tracking-wide text-gray-500">Địa phương</label>
          <input id="ol-location" type="text" value={location} onChange={e => setLocation(e.target.value)}
      ...
      <div className="flex flex-col px-4 py-2.5 gap-1.5">
        <label className="text-[16px] font-bold uppercase tracking-wide text-gray-500">
          Loại ơn lành{" "}<span className="normal-case font-normal text-gray-400">(có thể chọn nhiều)</span>
        </label>
      ...
      <div className="flex flex-col px-4 py-2.5 gap-1 focus-within:bg-blue-50/30 transition-colors">
        <label htmlFor="ol-title" className="text-[16px] font-bold uppercase tracking-wide text-gray-500">
          Tiêu đề lời chứng <span className="text-red-500">*</span>
        </label>
        <input id="ol-title" type="text" required value={title} onChange={e => setTitle(e.target.value)}
          placeholder="Tóm tắt ngắn gọn"
          className="text-[18px] text-vatican-dark placeholder:text-gray-300 outline-none bg-transparent" />
      </div>

      <div className="flex flex-col px-4 py-2.5 gap-1 focus-within:bg-blue-50/30 transition-colors">
        <label htmlFor="ol-content" className="text-[16px] font-bold uppercase tracking-wide text-gray-500">
          Nội dung lời chứng <span className="text-red-500">*</span>
        </label>
        <textarea id="ol-content" rows={6} required value={content} onChange={e => setContent(e.target.value)}
          placeholder="Kể lại câu chuyện của bạn một cách trung thực và chi tiết..."
        ...
        <div className="flex-1">
          {error ? (
            ...
            <p className="text-[16px] text-gray-400 leading-snug">
              Bằng cách gửi, bạn đồng ý để Ban biên tập xét duyệt và đăng tải lời chứng.
            </p>
          )}
        </div>
